...
def corregir_marcas(dataset:pd.DataFrame) -> pd.DataFrame:
    """
    Unificacion de los strings del feature 'Marca' para evitar duplicados y correccion de aquellos mal escritos (por ejemplo, 'hiunday': 'Hyundai')
...
def tratar_motor(dataset: pd.DataFrame) -> pd.DataFrame:
    """
    Extrae informacion estructurada a partir del texto del feature 'Motor'.
    Genera variables numericas y binarias que capturan la cilindrada y el tipo de motor, y elimina la columna original
    
    Variables creadas:
    - Motor_Litros: cilindrada extraida
    - Motor_Turbo: 1 si el motor es turbo (por ejemplo, del tipo turbo, tsi, tdi, etc)
    - Motor_Multipunto: 1 si el motor es de inyeccion multipunto
    - Motor_Diesel: 1 si el motor es del tipo Diesel
    - Motor_Litros_Faltante: 1 si no se pudo extraer la cilindrada del motor del texto
...
def clasificar_version(dataset: pd.DataFrame) -> pd.DataFrame:
    """
    Clasifica la version del vehiculo en una categoria de gama: Premium, Intermedia o Base, segun palabras clave presentes en el feature 'Version' (categorico).
    Elimina la columna original

    Variables creadas:
    - Version_Premium: 1 si la version contiene palabras clave de esta gama alta (por ejemplo: 'premium', 'executive', 'titanium', etc)
    - Version_Intermedia: 1 si la version contiene palabras clave de esta gama media (por ejemplo: 'allure', 'xls', 'advance', etc)
    - Version_Base: 1 si no se detectaron palabras clave de ninguna de las categorias previas
...
def unir_colores(dataset:pd.DataFrame) -> pd.DataFrame:
    """
    Estandariza la columna 'Color' agrupando variantes, sinónimos y nombres en otros idiomas
    bajo una única categoría general (ej. 'summit white' y 'blanca' pasan a 'blanco').
...
def completar_color_descripcion(dataset: pd.DataFrame) -> pd.DataFrame:
    """
    Completa valores nulos de la columna 'Color' buscando menciones explícitas de colores
    dentro del texto de la columna 'Descripción'. Solo reemplaza el valor cuando el color
    está ausente y se encuentra una coincidencia exacta de palabra en la descripción.
...
def tratar_camara_retroceso(dataset: pd.DataFrame) -> pd.DataFrame:
    """
    Completa valores nulos de 'Con cámara de retroceso' buscando menciones explícitas en el texto de la columna 'Descripción' (ej. "cámara de retroceso", "sensores", 
    "parking assist").
    Si la descripción menciona alguno de estos patrones, se asigna "Sí".
    Los valores que siguen sin poder determinarse se completan como "Desconocido"
...
def descripcion_scoring(dataset:pd.DataFrame) -> pd.DataFrame:
    ...
    palabras_positivas = [
        ...
        'tratamiento cerámico', 'tratamiento ceramico',
        # Llaves / documentación
        'dos llaves', '2 llaves', 'juego de llaves', 'manual de fábrica', 'manual de fabrica',
        # Mecánica reciente
        'fluidos recientemente cambiados', 'tensores nuevos', 'tensor nuevo',
        'bomba de nafta nueva', 'bomba de combustible nueva',
        # Neumáticos seminuevos
        'cubiertas seminuevas', 'gomas seminuevas', 'neumáticos seminuevos', 'neumaticos seminuevos',
        # Kilometraje de ruta
        'km de ruta', 'kilómetros de ruta',
        # Sin ruidos
        'no hace un ruido', 'no hace un ruidito', 'no hace ruidos',
        # Auxilio sin rodar
        'auxilio sin rodar',
    ]
    ...
    palabras_negativas = [
        ...
        # Motor con problemas
        'motor echo a nuevo pero no funciona', 'motor hecho a nuevo pero no funciona',
    ]
...
def preprocesamiento_pre_split(dataset:pd.DataFrame) -> pd.DataFrame:
    """
    Aplica el preprocesamiento inicial al dataset completo, antes de realizar el split en entrenamiento y validación (por eso se aplican transformaciones basadas únicamente en 
    los datos de cada muestra, sin riesgo de data leakeage)
    ...
    dataset = convertir_a_usd(dataset, tipo_de_cambio = 1172.5)
...
def completar_color_sets(X_train: pd.DataFrame, X_val: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Completa los valores nulos de 'Color' en entrenamiento y validación usando la moda por
    'Marca_Modelo' calculada únicamente sobre train
    Si una combinación de Marca_Modelo no está presente en train o el valor sigue siendo nulo, se completa con la moda global de 'Color' en train
        ...
        Parámetros de salida:
            X_train(pd.DataFrame): matriz de entrenamiento con 'Color' sin nulos
            X_val(pd.DataFrame): matriz de validación con 'Color' sin nulos
    """
...
def crear_features_autos(set:pd.DataFrame, año_actual:int = 2025) -> pd.DataFrame:
    ...
    - Log_Kilómetros: logaritmo de la cantidad de kilometros con los que cuenta el vehiculo
